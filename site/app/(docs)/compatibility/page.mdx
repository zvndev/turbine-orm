...
<Callout type="warning" title="turbine migrate does not read config.adapter yet">
  ...
  adapter below, and to its statement-timeout and row-estimate overrides:
  the adapter objects are complete, the config wiring is not.
</Callout>
...
| Statement timeout | `set_config('statement_timeout', …, true)` | `set_config('transaction_timeout', …, true)` (v23.1+) |
...
| `statement_timeout` | Not supported | Adapter uses `set_config('transaction_timeout', …)` |
