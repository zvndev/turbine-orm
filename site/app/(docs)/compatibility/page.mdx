...
Turbine v0.12 extends the internal `Dialect` interface seam across DML generation. PostgreSQL remains the only GA target in the `turbine-orm` package, but the query builder now routes SQL primitives like quoting, placeholders, JSON aggregation, insensitive search, JSON filters, and DML generation (`RETURNING`, `UNNEST`, `ON CONFLICT`) through a dialect contract so future `@turbine-orm/mysql` and `@turbine-orm/sqlite` packages can implement native SQL without compromising the Postgres path.
