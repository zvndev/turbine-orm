...
Turbine v0.13 extends the internal `Dialect` interface seam across query generation, DML generation, schema DDL, and migration tracking SQL. PostgreSQL remains the only GA target in the `turbine-orm` package, but SQL primitives like quoting, placeholders, JSON aggregation, DML (`RETURNING`, `UNNEST`, `ON CONFLICT`), column types, `CREATE TABLE`, indexes, and `_turbine_migrations` statements now route through a dialect contract so future `@turbine-orm/mysql` and `@turbine-orm/sqlite` packages can implement native SQL without compromising the Postgres path.
