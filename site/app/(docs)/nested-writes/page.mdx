...
"Relations" above means **to-one and one-to-many** (`belongsTo`, `hasOne`, `hasMany`). **Many-to-many relations have no nested-write support on any engine**: a relation op on an m2m relation throws a `ValidationError` ([E003](/errors)) naming the junction table, rather than silently doing nothing. Write junction rows directly (`db.userTags.createMany(...)`) inside the same `$transaction`.
...
- **Validated.** Update-only ops in `create()`, unknown op names, non-nullable `disconnect`, missing `connect` targets, and any op on a many-to-many relation all throw `ValidationError` ([E003](/errors)) before anything commits.
