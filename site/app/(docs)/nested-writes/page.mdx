...
A key in `data` is treated as a nested write when it matches a relation name and its value is a plain object (not `null`, not an array, not a `Date`). Everything else is a scalar column. There is no flag to set.
...
Many-to-many relations support `connect`, `disconnect`, and `set` only; the full rules are in [Many-to-many relations](#many-to-many-relations) below.
...
For a `belongsTo` relation the foreign key lives on the row you're creating, so Turbine resolves the related row **before** the parent INSERT and folds its key in. A `NOT NULL` FK column is therefore satisfied on the first insert.
...
This handler has an authorization check, and the check is on the wrong row:
...
It is off by default because it changes the outcome of writes that currently succeed. Turn it on in a non-production environment first and run your test suite: anything that legitimately re-parents will start throwing, and that is the list of call sites to rewrite as explicit updates.
...
- **`connect` is idempotent.** Turbine reads the parent's existing links inside the transaction and inserts only the missing ones, rather than relying on `ON CONFLICT DO NOTHING` (which SQL Server and PowDB refuse, and which assumes a unique constraint an implicit junction need not declare). Connecting an already-linked tag is a no-op, not a duplicate row and not a constraint error.
