...
`includePii` is only valid on reads (`findMany` / `findUnique` / `findFirst` and their `*OrThrow` forms, plus `findManyStream`); it is not a write option. Filtering or ordering by a PII column (`where`, `orderBy`, `groupBy`, aggregates) is always allowed regardless of the flag, since naming the column is itself explicit. Schemas with no PII-tagged columns are unaffected: the emitted SQL is byte-identical either way.
