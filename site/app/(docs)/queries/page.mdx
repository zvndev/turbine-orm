...
One boundary worth knowing: writes still use `RETURNING *`, so PII values in the affected row do transit the database connection before Turbine strips them from the returned object. The guarantee is that PII never reaches your application code unrequested, not that it never crosses the wire.
