...
**Upgrade note (0.73.0).** Before this release `skip` was not a Turbine key at all while `take` was, so the Prisma pair `{ take: 20, skip: 40 }` type-checked, ran, and silently returned the FIRST page however far you thought you had paged. If you have code written that way, it is now paging correctly, which may change what it returns.
</Callout>
