...
`mode: 'insensitive'` applies to `equals`, `not`, `in` and `notIn` on string columns as well as to the substring operators; see [String](#string). Before 0.78 it was read only by `contains` / `startsWith` / `endsWith` and silently ignored beside the others. On `in` / `notIn` it is PostgreSQL-only and every other engine refuses it with `TURBINE_E017`, for the reason given in the table below.
