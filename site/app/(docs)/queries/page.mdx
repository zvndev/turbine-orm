...
[errors page](/errors#unknown-field-names) for the relation-name case, which has
its own message.
...
`includePii` is a [privilege option](/global-filters#privilege-options-and-the-unsafe-symbol): the `UNSAFE` symbol is the only value that enables it, and a literal `includePii: true` throws a `ValidationError` (`TURBINE_E003`). The reason is that `includePii` sits on the same options object as `where`, so a handler written as `db.users.findMany({ ...req.body })` used to let a request body ask for the PII columns; `JSON.parse` cannot produce a symbol. Read [Privilege options](/global-filters#privilege-options-and-the-unsafe-symbol) for the full rationale and the upgrade table.
...
<Callout type="warning" title="PostgreSQL only">
  `DISTINCT ON` is a PostgreSQL extension that no other engine parses. Since
  0.66 `distinct` is refused on SQLite, MySQL and SQL Server with
  `UnsupportedFeatureError` (`TURBINE_E017`), the same typed error the other
  spelling of this feature (`groupBy({ distinctOn })`) has always thrown. Before
  0.66 the clause was emitted with no dialect check at all, so those engines
  answered with a raw driver parse error (`near "ON": syntax error`,
  `ER_PARSE_ERROR`) carrying no `TURBINE_*` code. The PostgreSQL SQL is
  unchanged. CockroachDB and YugabyteDB run on the PostgreSQL dialect, so the
  gate does not fire for them and the clause is emitted as-is.
</Callout>
...
`allowFullTableScan` is a [privilege option](/global-filters#privilege-options-and-the-unsafe-symbol): `true` throws (`TURBINE_E003`), because on an options object built from a request body a plain boolean turns "delete the rows I named" into "delete the table". The option is typed `Unsafe`, so a boolean does not compile at all, `false` included; write a conditional call site as `{ where, ...(flag ? { allowFullTableScan: UNSAFE } : {}) }`. (`false` / `null` / `undefined` are still accepted at *runtime* and mean "guard on", which keeps untyped callers working.) The same applies to `updateMany`.
...
<Callout type="note" title="Why the speculative fetch stops above batchSize 1000">
  When the result does **not** fit in one batch, those speculatively fetched
  rows cannot be reused. They were read outside the cursor's transaction, so
  yielding them and then continuing from the cursor would splice two snapshots
  together, and resuming past them would need an `ORDER BY` you never asked
  for. The cursor therefore re-reads from row one, and the speculative fetch
  cost `batchSize + 1` rows for nothing.

  That waste is proportional to `batchSize`, which is the one number you raise
  when you expect **more** rows, so raising it for throughput used to make
  large drains slower: over 50,000 rows, 56.98 ms at `batchSize: 1000` and
  59.55 ms at 5000, while every other way of reading the same rows got faster.
  Above the default the speculation is now skipped and the cursor is opened
  directly, so nothing is fetched twice. The cost of being wrong about that is
  four round trips on a drain that would have fit, never any transferred rows.
</Callout>
...
<Callout type="note" title="planCacheMode: when a query gets slower on its sixth execution">
  ...
  **Deep pagination is the other common instance, and it surprises people
  because there is no skewed column involved.** `limit`/`offset` are bound as
  parameters, so a generic plan cannot know the offset is 9,000 rather than 40,
  and it plans the page as if it were shallow. On the benchmark fixture, a
  20-row page of filtered, sorted posts costs 0.212 ms at offset 40 and **0.549
  ms at offset 9,000**; the same query with `forceCustomPlan: true` costs 0.292
  ms, within 6% of hand-written `pg`. Nothing about the SQL changed. If you
  paginate deeply on a hot endpoint, set `forceCustomPlan: true` on that query.

  This is a property of naming prepared statements, not of Turbine: a
  hand-written `pg` query with a statement `name` reproduces it exactly (0.541
  ms named against 0.272 ms unnamed), and ORMs that never name statements never
  see it, which is also why they do not get the `Parse` skip that makes repeated
  point reads fast. `benchmarks/deepoffset.ts` runs all four arms.
