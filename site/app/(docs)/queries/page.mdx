...
When `warnOnUnlimited` is enabled, a `findMany` with no `limit` / `take` / `cursor` logs a one-time warning. Silence it for a query you know is bounded:
...
`cursor` and `distinct` queries are exempt (both impose their own ordering semantics). The warning is gated exactly like `warnOnUnlimited` (per-call, per-table, global), deduped per table and shape, and silent under `NODE_ENV=production`.
...
For deep pagination, `cursor` is the keyset alternative: pass the column values of the last row you saw, and Turbine adds one comparison per cursor field (`>` for ascending order, `<` for descending, direction taken from `orderBy`). The cursor row itself is **excluded**, so the next page starts immediately after it:
...
Multiple cursor fields combine with `AND`. Use a unique (or unique-in-combination) column in `cursor` + `orderBy` so the ordering is total; unlike `OFFSET 100000`, a keyset page never scans the skipped rows.
...
Each extra hop becomes an INNER JOIN inside the *same* correlated subquery: one subquery, one `LIMIT 1`, however many hops. Every hop applies its target's global filter to the join condition, so ordering never keys off a soft-deleted or other-tenant row.

A **to-many** hop mid-chain is refused: it has no single value to order by. Use a pick-row ordering (`{ pick, by }`) or `_count` instead.
...
The result row is **fully typed** from the args, no `as const` needed: each `by` field carries its entity field type, `_count` is a `number`, `_sum` / `_avg` fields are `number | null` (an aggregate over zero matching rows is null), and `_min` / `_max` fields carry the field's own type. The call above infers:
...
| `implicitPkOrdering` | `false` | Order a paginating `findMany` that declares no `orderBy` by the primary key ascending, making its pages [deterministic](#deterministic-pages-the-unordered-pagination-warning). An explicit `orderBy` wins; PK-less tables, `cursor` and `distinct` shapes are untouched. |
