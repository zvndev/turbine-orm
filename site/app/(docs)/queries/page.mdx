...
// → ORDER BY ("data" #>> $1::text[])::numeric ASC
```

Works top-level and inside a `with` relation's `orderBy`. (Ordering a parent by a JSON path in a *related* row is on the roadmap — today that still needs `client.sql`.)
