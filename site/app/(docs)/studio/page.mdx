...
From inside the UI you can flip between the three Studio modes live: a **Read-only / Show PII / Write** switcher in the demo banner toggles PII redaction and single-row writes independently, so you can try the redaction behavior and the write flow back to back without restarting. Every launch boots read-only with PII redacted.
