...
Zone-less temporal cells render the way your application reads them. Studio builds its own pool rather than a `TurbineClient`, and before v0.54 it kept the pg driver's local-zone parsers, so east of UTC a `date` cell displayed the previous evening while the app reading that same row saw UTC midnight. In `--write` mode the displayed value is also what an edit echoes back into the column, so the mismatch could shift a stored day. Studio now registers the same UTC parsers the client does (`timestamp`, `date` and their array forms), from one shared helper so the two cannot drift. `turbine mcp` gets the same treatment for the rows it samples. See [Zone-less columns](/schema#zone-less-columns-timestamp-and-date-read-as-utc).
