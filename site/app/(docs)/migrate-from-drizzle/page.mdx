...
Drizzle and Turbine both resolve a nested read in a single statement: Drizzle's relational queries compile to `LEFT JOIN LATERAL` plus JSON aggregation, Turbine's `with` compiles to correlated `json_agg` subqueries. The difference is the surface. Turbine is a Prisma-style typed API with schema-generated types (no `relations()` re-declaration), typed errors, a read-only-by-default Studio, PII enforced in the emitted SQL, and edge/serverless support with no config.
...
Turbine's `with` generates a single SQL statement using `json_agg` subqueries. No N+1, no separate round-trips.
...
| Typed errors with codes | Generic Error (raw pg errors) | Typed error classes (`TURBINE_E001`-`E018`) with SQLSTATE mapping |
...
- **First-class multi-database support.** Drizzle treats MySQL, SQLite, and Postgres as peers. Turbine is Postgres-first: SQLite, MySQL 8, SQL Server, and PowDB ship behind subpath exports (`turbine-orm/sqlite`, `/mysql`, `/mssql`, `/powdb`) with the same typed query API, but a handful of query features throw a typed `UnsupportedFeatureError` there (pgvector, LISTEN/NOTIFY, RLS session context), and the schema tooling (`turbine generate` / `push` / `migrate`, `doctor`) targets Postgres, so the other engines are code-first via `defineSchema`. See [What is actually Postgres-only](/engines#what-is-actually-postgres-only).
