...
Drizzle is a great TypeScript ORM with a SQL-like query builder, and both libraries resolve a nested read in a single statement: Drizzle's relational queries compile to `LEFT JOIN LATERAL` plus JSON aggregation, Turbine's `with` compiles to correlated `json_agg` subqueries. That is not the difference between them. What Turbine adds is a Prisma-inspired typed surface with schema-generated types (no `relations()` re-declaration), typed errors, a read-only-by-default Studio, PII enforced in the emitted SQL, and edge/serverless support with no config. If you want the ergonomics of `findMany({ with: { posts: true } })` and those defaults, Turbine is a natural step.
...
|---|---|---|
| Single-query nested relations | Yes, relational queries (RQBv2) | Yes, via `json_agg`, typed and date-coerced at any depth |
| Typed errors with codes | Generic Error (raw pg errors) | 16 error classes with SQLSTATE mapping |
...
| Down migrations | None, `drizzle-kit drop` removed in v1 | SQL `-- DOWN` sections, checksummed |
| Local Studio UI | Drizzle Studio (free; hosted Gateway paid) | `npx turbine studio` (free, read-only) |
| Runtime deps | 0 (pg adapter separate) | 1 (`pg`) |
| Select/omit type narrowing | Yes (via `.select()`) | Yes (via `select`/`omit` args) |
| Empty-where guard | No protection | Rejects `{}` unless [`allowFullTableScan: UNSAFE`](/global-filters#privilege-options-and-the-unsafe-symbol) |

## What you'll lose

- **SQL-like syntax.** If you prefer `select().from().where()` chaining, Turbine's object API is a style change.
- **First-class multi-database support.** Drizzle treats MySQL, SQLite, and Postgres as peers. Turbine is **Postgres-first**: PostgreSQL (plus the wire-compatible CockroachDB and YugabyteDB) is the primary target, and SQLite, MySQL 8, SQL Server, and PowDB ship behind subpath exports (`turbine-orm/sqlite`, `/mysql`, `/mssql`, `/powdb`) sharing the same typed query API. What you give up on those engines is narrower than "no support": a handful of query features throw a typed `UnsupportedFeatureError` (pgvector, LISTEN/NOTIFY, RLS session context), and the schema tooling (`turbine generate` / `push` / `migrate`, `schemaPush` / `schemaDiff`, `doctor`) targets Postgres, so the other engines are code-first via `defineSchema`. See [What is actually Postgres-only](/engines#what-is-actually-postgres-only) for the exact line.
