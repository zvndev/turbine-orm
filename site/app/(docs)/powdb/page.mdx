...
[PowDB](/engines#powdb) is a single-node database with its own query language, PowQL, which shares no surface with `SELECT ... FROM ... WHERE`. It is the one engine that bypasses the [`Dialect`](/dialects) contract for **query generation**: the module ships a parallel query generator (`PowqlInterface`) with the same public method surface as the SQL `QueryInterface`, wired in through the `queryInterfaceFactory` seam.
...
<Callout type="note" title="No driver in the base install">
  ...
  `import()` only when you open a PowDB connection:
...
<Callout type="warning" title="assumeEngineVersion is a promise you are making">
  Passing `assumeEngineVersion: '0.19.1'` to a server actually running 0.13
  does not make the features work. It removes the guard that would have given
  you a clear `TURBINE_E017` and leaves you with a raw PowQL parse error.
  Use it only when the version genuinely cannot be probed.
</Callout>
...
`wrapPowdbError(err)` maps both transports into Turbine's [typed error hierarchy](/errors). It handles two very different inputs: the embedded napi addon tags **every** error `code: 'GenericFailure'`, so there is nothing to switch on there, while a networked engine 0.17+ carries a typed wire error class.
...
- **A re-entrant `db.$transaction`**, one opened inside an active transaction callback's async context, throws `TURBINE_E017`: queueing it would deadlock against the transaction waiting for it. Detection uses `AsyncLocalStorage`, scoped to the callback's async subtree, so a same-tick sibling transaction is correctly treated as independent.
