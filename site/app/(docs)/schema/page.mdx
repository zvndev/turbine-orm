...
<Callout type="warning" title="Reads still take a string">
  This is a write-path coercion only. A `Date` in a `where`, `orderBy`, or
  `having` clause on a `time` / `timetz` column is **not** narrowed: it is bound
  as a full timestamp and Postgres rejects it (`22007 invalid input syntax for
  type time`). Compare with a string there: `where: { startsAt: { gte:
  '09:00:00' } }`.
</Callout>
