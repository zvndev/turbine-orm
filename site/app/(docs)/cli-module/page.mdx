...
| `adapter` | none | A `DatabaseAdapter` for a PG-compatible database that needs dialect-specific behaviour, e.g. `cockroachdb`. **Type only, the bundled CLI does not read it yet.** See the note below. |

<Callout type="warning" title="adapter is declared on the type but the bundled CLI ignores it">
  `TurbineCliConfig.adapter` type-checks, and Turbine's internal migration
  runner takes an adapter, but `turbine migrate` never forwards the config
  field into it. So `adapter: cockroachdb` in `turbine.config.ts` changes
  nothing: migrations still take the PostgreSQL path, including
  `pg_try_advisory_lock()`, which CockroachDB does not implement. Until this
  is wired, do not rely on the CLI to hold a cluster-wide migration lock on
  CockroachDB or YugabyteDB. Serialize `turbine migrate up` yourself (one
  runner, in a CI job that cannot overlap), or apply the generated `.sql`
  files with a tool that supports the engine. The `adapter` value is otherwise
  inert, not harmful. See [Compatibility](/compatibility).
</Callout>
