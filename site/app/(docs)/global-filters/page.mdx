...
A global filter is a `WHERE` predicate Turbine merges into **every** query on a table: reads, mutations, and the relation subqueries that target it. Two patterns motivate it:
...
Global filters apply to **relation subqueries** too. A filter on `posts` restricts `posts` wherever it appears, including when loaded through a parent's `with` clause.
...
A target-table filter is applied to:
...
So a soft-delete filter on `posts` means a user's post `_count` counts only live posts, and `where: { posts: { some: {...} } }` matches only live posts.

## Mutations and the empty-where guard

Global filters apply to `update`, `updateMany`, `delete`, `deleteMany`, and the conflict-`UPDATE` of `upsert`. `create` and `createMany` are **never** filtered: an insert has nothing to scope.
...
```ts
// The shape this closes: a request body spread straight into query args.
...
This is a **breaking change** on those three options, and deliberately loud.
...
**TypeScript finds every one of them for you.** The option types are `Unsafe`, so each row of that table is a compile error before it is a runtime error: type-check the project and the compiler hands you the complete list of call sites to change.

That includes the boolean shapes: `allowFullTableScan: false` does **not** typecheck, and neither does `allowFullTableScan: someFeatureFlag`. A conditional call site adds the key or not:
