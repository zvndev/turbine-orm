...
<Callout type="warning" title="Do not force 'batched' by benchmarking on localhost">
  ...
  on the server. Whether that trade pays depends entirely on what a round trip
  costs you, and a Unix socket or a container on the same host makes it look
  free. Measured on one fixture (50 users with their posts) through a proxy that
  injects latency:
  ...
  is 1.83x slower. `'auto'` staying on the join here is the right call for
  anything talking to a database over a network, which is nearly everything in
  production. If your deployment is not on a socket, tell the client so with
  `autoRoundTripMs` (see the tuning note below) rather than pinning a strategy,
  and benchmark on a link that resembles production. Reproduce with
  `benchmarks/bench-latency.ts`.
</Callout>
...
> **Tuning note.** The two cardinality/indexing rules above are heuristics: the real parent count is unknown until the base query runs, and `limit` is the only bound available at plan time. If profiling disagrees with the choice, raise or lower `autoToOneJoinMaxRows` (which governs the to-one rule only), or pin `relationLoadStrategy` on that query.
>
> **`autoRoundTripMs`** is the other half of the same dial, and the one worth setting first. The to-one threshold is derived from it, because the whole question `'auto'` is answering is whether an extra round trip costs less than re-evaluating a correlated subquery per parent row, and that depends entirely on your link. Set it to what `ping` says between your app and your database: roughly `0.05` for a Unix socket, `0.5` to `2` for same-region managed Postgres, `30` to `60` cross-region. It defaults to `0.7` (same-region), which reproduces the historical 1000-row threshold exactly. An explicit `autoToOneJoinMaxRows` overrides it, and it is consulted only under `'auto'`. The break-even it governs moves about **17x** between a loopback link and a 2.7 ms one, so on anything that is not same-region this is the single highest-value line in your client config.
>
> ```ts
> const db = turbine({ connectionString: process.env.DATABASE_URL, autoRoundTripMs: 1.2 });
> ``` On endpoints that fan out to very large child sets (megabyte-scale JSON per request), the batched loader's flat rows can beat the join plan even with healthy indexes and a small parent set, so `'batched'` is still worth pinning there.
...
Because `'auto'` never selects it, this is the one strategy you have to ask for. On the benchmark fixture (500 posts each with their author) it is **1.53x faster than the default plan**: 1.734 ms against 2.658 ms. Unlike `'batched'` it spends **no extra round trip**, so that margin is not an artifact of measuring on a socket and does not erode as latency rises. If your hot query reads a to-one relation, this is worth trying.
