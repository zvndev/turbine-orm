...
Relation `_count` follows the unindexed rule only, and likewise only on a large parent set: a grouped `COUNT(*) ... GROUP BY fk` scans the child table once either way, so on a bounded query the follow-up round-trip costs more than it saves.

> **Tuning note.** The two rules are heuristics: the real parent count is unknown until the base query runs, and `limit` is the only bound available at plan time. If profiling disagrees with the choice, raise or lower `autoToOneJoinMaxRows`, or pin `relationLoadStrategy` on that query. On endpoints that fan out to very large child sets (megabyte-scale JSON per request), the batched loader's flat rows can beat the join plan even with healthy indexes and a small parent set, so `'batched'` is still worth pinning there.
