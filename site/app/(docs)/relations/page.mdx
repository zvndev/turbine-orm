...
Stated narrowly, because it was measured rather than assumed. The numbers above come from a named prepared statement over the client's own pool, and the option demonstrably reaches every connection the client hands out. What it does **not** do is make every Turbine query vulnerable in the first place:

- **`count()` is the exposed shape.** It compiles to `SELECT COUNT(*) FROM t WHERE …` with the predicate parameterized and nothing else, so a skewed predicate is exactly the promotion candidate described above.
- **`findMany` / `findFirst` are much less exposed**, because they bind the limit as a parameter (`LIMIT $n`). A parameterized limit denies the planner the limit fraction that makes the ordered-scan plan look cheap, and the same query shape driven through `findFirst` stayed flat across all nine executions in the run above.

So treat `planCacheMode` as a targeted remedy for a statement you have **measured** getting slower after its fifth execution, not as a general speed-up. Setting it globally on a workload with no skew buys you nothing and costs you a re-plan per execution.
