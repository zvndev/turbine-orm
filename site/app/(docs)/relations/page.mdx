...
When you don't set `relationLoadStrategy`, Turbine uses **`'auto'`** (since 0.41.0): it compiles the single-statement correlated-subquery plan (`'join'`), but when schema metadata shows a relation's correlation column has **no covering index**, that relation falls back to the batched loader instead of running an unindexed probe per parent row. Engagement is visible: a once-per-relation dev note, and query events carry `strategy: 'auto-batched'`. An explicit `'join'` or `'batched'` (client-wide or per query) always wins over `'auto'`.

> **Tuning note.** `'auto'` only falls back when an index is provably missing; it keeps the join plan for indexed relations. On endpoints that fan out to very large child sets (megabyte-scale JSON per request), the batched loader's flat rows can still beat the join plan even with healthy indexes. If profiling shows that, pin `relationLoadStrategy: 'batched'` on that query; the output is deep-equal either way.
