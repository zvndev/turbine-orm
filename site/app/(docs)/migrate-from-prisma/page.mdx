...
**Do not read the last two rows as "correlated beats batched once indexed."** An earlier version of this page made that argument, and it does not generalize: on a to-one relation over a local socket, the batched loader is measurably faster than the correlated plan at every parent-set size from about **25 rows** upward, and the gap widens with parent count. Turbine's own `'auto'` default no longer follows the correlated-wins advice either: above `autoToOneJoinMaxRows` (default 1000) it switches a to-one relation to the batched loader precisely because the correlated plan loses at scale.

### The real tradeoff

The two plans trade a different cost, and neither wins everywhere:

- **Correlated (`'join'`) is one round-trip**, but the subquery is re-evaluated **once per parent row**. Its cost scales with parent-set size, even on a perfect index seek. Measured on a local socket, that per-parent cost is roughly **0.0016 ms per parent row**.
...
So the crossover is wherever one round-trip equals the correlated plan's per-parent cost, which makes it depend on **both** parent-set size and your round-trip time:

| Deployment | Round trip | Correlated wins below roughly |
|---|---|---|
| Local socket / same host | ~0.03 ms | 25 parent rows |
| Same-region managed Postgres | ~1 ms | 600 parent rows |
| Cross-region pooled connection | ~35 ms | 22,000 parent rows |

The default threshold of 1000 is the right answer for a database about 1.6 ms away, which is a reasonable guess at same-region managed Postgres. It is far too high for a local socket and too low for a cross-region pooled connection. **If your database is on the same host or a very low-latency link, lower `autoToOneJoinMaxRows` substantially** (into the tens) or pin `relationLoadStrategy: 'batched'` on wide-parent queries. If it is far away, raise it.
...
| `'flatten'` *(0.50)* | Compiles eligible **to-one** relations to a `LEFT JOIN` in the same statement: one round-trip, no correlated re-evaluation, no client-side stitching. Anything ineligible silently falls back to a correlated subquery. | One round-trip is a hard requirement and the relations are to-one. See [Load strategies](/relations#load-strategy-auto-join-batched-flatten) for the eligibility rules and where it loses to `'batched'`. |
...
The fingerprint ignores differences a checkout can introduce on its own (line endings, a byte-order mark, trailing whitespace at end of file) and nothing else. An edited comment counts as a change: a regeneration you did not strictly need costs you nothing, and a missed one is the whole failure this exists to end.
...
The many-to-many rules changed in 0.50 (see [Notable differences](#notable-differences)), so a port needs to know which call sites are affected. The obvious recipe does not work.
...
<Callout type="warning" title="Before 0.57.0 these were silently dropped">
  This is the correction, stated plainly rather than as an improvement. Until
  0.57.0 the adapter built a fresh Turbine args object and copied a hand-written
  allowlist of keys. Anything not on that list was accepted by the type-checker
  and thrown away with no error, no warning and no test: `forceCustomPlan` (which
  0.56.0 shipped and `turbine doctor` recommended), `warnOnUnlimited`,
  `skipGlobalFilters`, `allowFullTableScan`, `timeout` on every write except
  `create`, `optimisticLock`, `stableRelationOrder` and `distinctOn`.

  Two of those changes are visible on upgrade even if you change nothing:
  **`skipGlobalFilters` now takes effect** where it used to be inert, and
  **`relationLoadStrategy: 'query'`** now maps to Turbine's `'batched'` instead of
  being forwarded to a resolver that had no `'query'` branch and silently used the
  join plan. `limit` on `updateMany` / `deleteMany` now throws
  `UnsupportedFeatureError` (`TURBINE_E017`) rather than being dropped, because
  silently ignoring a bound on a mass mutation is the worse failure.

  The class of bug is closed rather than the instance: the option surface is now
  runtime data (`src/query/option-surface.ts`) that the compiler checks against
  the core arg interfaces, so adding an option to core **fails the build** in the
  adapter until it is classified. It does not make one edit sufficient; it makes
  the missing second edit loud.
</Callout>
