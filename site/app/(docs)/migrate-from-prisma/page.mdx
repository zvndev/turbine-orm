...
| `prisma.user.aggregate` | `db.users.aggregate` | `_sum` / `_avg` / `_min` / `_max` / `_count`. `_count: true` returns a number; `_count: { _all: true }` returns `{ _all: n }` (Prisma's shape). Over `int8` / `bigint` and `numeric` / `decimal` columns, `_sum` / `_avg` come back as exact **strings** where Prisma hands you a `Decimal`, the same rule as the `numeric` row in [Silent value differences](#silent-value-differences-check-these-first); over `int4` / `float` columns they are numbers. |
