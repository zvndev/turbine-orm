...
One step is mandatory on either path before you benchmark: run `npx turbine doctor`, covered next.
...
If you generated a client before 0.41, this one changes types on you. When a child table's foreign-key columns are exactly covered by a unique constraint or unique index, the relation is genuinely one-to-one, and introspection now emits **`hasOne`** for the parent side instead of `hasMany`. In the generated types, `Child[]` becomes `Child | null`:

```ts
// Before 0.41
const user = await db.users.findUnique({ where: { id: 1 }, with: { profile: true } });
user.profile[0]?.bio;   // array, always length 0 or 1
