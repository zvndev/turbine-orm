...
<Callout type="warning" title="Three of those are privilege options and need the UNSAFE symbol">
  ...
  The adapter forwards them **verbatim**, on purpose, so core stays the single
  judge and compat carries no second copy of the rule. That means a compat call
  site written `{ skipGlobalFilters: true }` throws a `ValidationError`
  (`TURBINE_E003`) exactly like a native one. Import the sentinel and pass it:
  ...
  This matters more here than on the native API, because a compat client is
  usually reached from handlers that were written against Prisma, where every
  query option is a plain JSON value.
</Callout>
