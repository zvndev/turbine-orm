...
- **`timestamp` columns are UTC on both sides.** Turbine reads Postgres `timestamp` (without time zone) values as UTC, the same convention Prisma uses, so a migrated app gets **identical instants**, not values shifted by the server's local zone. Since v0.52 the same holds on the way in: the `Date` values that `create` / `update` / `upsert` and `where` clauses bind to zone-less `date` / `timestamp` columns are written as UTC too. Opt out with `utcTimestamps: false` if you relied on local-time handling, with two caveats. First, before v0.52 the flag reached the read path alone, so a client that had already set `false` was still binding UTC on writes and filters; on upgrade those statements change, and so does the **text stored** in zone-less columns, leaving pre-upgrade and post-upgrade rows on two conventions until you backfill. Second, the flag is **per process, not per client**: the read half is a pg type parser on OID 1114, which `pg.types.setTypeParser` installs once for the whole pg module, so the first Turbine-owned client settles it and a second Turbine-owned client asking for the opposite value throws `ValidationError` (`TURBINE_E003`) at construction rather than reading and writing in different zones. Clients on an external pool never register a parser and are exempt.
