...
`explain_query` never accepts free-form SQL. You pass builder args — the same shape Studio’s Query tab compiles — and Turbine validates identifiers against the introspected schema, builds parameterized SQL via `QueryInterface.buildFindMany`, then runs `EXPLAIN (FORMAT JSON)` on that SQL with bound params only.
