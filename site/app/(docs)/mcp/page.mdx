...
Ten read-only tools. Every database access runs inside a `BEGIN READ ONLY` transaction with a statement timeout, an agent physically cannot mutate your data through them.
...
| `sample_rows` | Up to 50 rows from a table whose name is validated against the introspected schema. PII-tagged columns come back redacted. |
...
`sample_rows` caps at 50 rows, validates the table name against the live schema, and **redacts PII-tagged columns** before the rows reach the agent's context (the response lists which columns were redacted). Tags are a code-first declaration that introspection never infers, so the server reads them from the generated metadata in your `out` directory: with no generated metadata there is nothing to redact, and rows come back whole. Together, an agent can explore query plans and real data without a path to writing or to arbitrary SQL.
