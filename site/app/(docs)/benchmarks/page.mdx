export const metadata = {
  title: 'Benchmarks',
  description:
    'Turbine ORM vs Prisma 7 vs Drizzle 0.45 on a real pooled Neon database. Honest numbers.',
...
> **Measurement vintage.** These numbers were measured **April 9, 2026** on Turbine **0.8.x** against the Prisma and Drizzle versions current at the time. Turbine's query-generation strategy (json_agg subqueries, template cache, prepared statements) is unchanged through 0.19.x, so we haven't re-run the suite for every release — but Prisma and Drizzle have shipped releases since (Prisma 7's engine-less client and JOIN-based relation loading have moved from preview toward default), so the relative numbers may have shifted. We'll re-measure and update this page; until then, treat the table as an April-2026 snapshot, not a current scoreboard.
...
- **The numbers are from April 2026.** Measured on Turbine 0.8.x; the query-generation strategy is unchanged through 0.19.x, but Prisma 7.6 and Drizzle 0.45 were the competitor versions at measurement time and both have moved since. We will re-measure rather than extrapolate.
