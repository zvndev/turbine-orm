...
> **TL;DR.** Over a local socket there is no network round-trip, so per-query overhead is the entire signal and most numbers are sub-millisecond. The single most useful figure on this page: **Turbine runs at 1.08x hand-written `pg`, where Drizzle runs at 1.47x and Prisma at 1.81x** (geometric mean over the eight scenarios with a raw control). Across all ten scenarios Turbine is **2.02x faster than Prisma 7.9** and **1.46x faster than Drizzle 0.45** by geometric mean. Turbine takes nine scenarios, Drizzle takes one (**streaming, which Drizzle wins outright by 22%**), and Prisma takes none. Three of the ten are contested and are claimed by neither side.
>
...
**Drizzle beats Turbine on streaming by 22%: 39.49 ms against Turbine's 50.39 ms** to drain 50,000 comments in batches of 1,000. This reproduced in every run across both harnesses, so it is not noise.
