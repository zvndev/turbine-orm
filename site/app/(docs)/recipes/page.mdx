...
Patterns for wiring Turbine into a real app. Each one is small: Turbine is a client you create once and call from your handlers.

## Next.js, singleton client + Route Handler

Create the client **once** and reuse it. Next.js hot-reload re-evaluates modules in development, which would leak a new connection pool on every save, so stash the instance on `globalThis`.
...
Import that `db` from any Route Handler. Don't call `db.disconnect()` per request: the pool is long-lived and shared.
...
The query layer never has to remember to filter by tenant; the policy does it.
