...
The [benchmark suite](/benchmarks) runs Turbine against Prisma, Drizzle, and a hand-written `pg` control on the same schema, data, and pool. In the last published run Turbine ran at **1.08x the raw `pg` control** by geometric mean, where Drizzle ran at 1.47x and Prisma at 1.81x. Turbine took nine of ten scenarios; the tenth, streaming, was Drizzle's in that run and has reversed in both re-measurements since.
