export const metadata = {
  ...
  description:
    'Turbine ORM is Postgres-first, with optional SQLite, MySQL 8, and SQL Server engines behind subpath exports. Same typed API, honest capability matrix, one root dependency.',
...
⁴ Optimistic locking throws `OptimisticLockError` on PostgreSQL, SQLite, and SQL Server. On MySQL (the `reselect` strategy) the write's row is re-fetched by `where` only, so a version conflict returns the current row instead of raising — depend on optimistic-lock failures throwing only on an engine with `RETURNING` / `OUTPUT`.
