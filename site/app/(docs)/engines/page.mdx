...
PowDB has no wire introspection, so you define the schema in code with `defineSchema` (there is no `turbine generate` for PowDB):
...
- **Nested `with` uses N+1 loaders, not a single query.** PowDB has no `json_agg`/link-navigation, so each relation level is a separate batched lookup (keys chunked at 1,000). Correctness is identical; depth-D nesting costs D round-trips, not one.
- **Primary keys are client-assigned.** PowDB does not generate IDs, so Turbine writes a UUID string PK on `create`. Plan your schema around string PKs (or supply your own).
- **Single-writer transactions, no nesting.** `db.$transaction` works, but PowDB has one global write lock — a nested or concurrent `$transaction` throws `UnsupportedFeatureError` (no savepoints) rather than deadlocking.
...
Beyond the Postgres-only trio every non-Postgres engine throws on (pgvector, LISTEN/NOTIFY, RLS `sessionContext`), PowDB also throws `UnsupportedFeatureError` for capabilities that are not yet built (several are slated for a later phase):

- **Filters:** JSON path/key, array, full-text, and pgvector-distance filters; vector / distance ordering.
- **Relations:** many-to-many relation filters *and* nested reads (junction tables — Phase B); composite-key relation filters and composite-key nested reads.
- **Writes:** nested writes inside `create` / `update` data (Phase B); composite-key `upsert`.
- **Reads:** cursor pagination and `findManyStream` (no server-side cursor — page with `findMany({ limit, offset })`).

Everything else — `findMany` / `findUnique` / `findFirst`, `create` / `createMany` / `update` / `updateMany` / `delete` / `deleteMany` / `upsert`, atomic update operators, `count` / `aggregate` / `groupBy`, `where` operators, `orderBy` / `select` / `omit`, and single-key `with` for hasMany / belongsTo / hasOne (including `some` / `none` relation filters) — works.
