...
All three SQL engine factories (`turbineSqlite`, `turbineMysql`, `turbineMssql`) take every `TurbineConfig` field **except** the connection and dialect plumbing they own (`pool`, `dialect`, `preparedStatements`, the pg connection / pool-tuning fields, and `replicas`, which has no engine equivalent). The forwarded set is defined by exclusion, so a new client option is engine-wide the day it lands rather than the day someone remembers to add it to three lists. That includes `logging`, `defaultLimit`, `warnOnUnlimited`, `errorMessages`, `logQueryParams`, `globalFilters`, `scopedConnect`, `utcTimestamps`, `relationLoadStrategy`, `stableRelationOrder` and the rest.

Options that are Postgres-only by **capability** rather than by plumbing (`jsonEncoding: 'positional'`) are forwarded too, and throw `UnsupportedFeatureError` (`TURBINE_E017`) when used. A typed refusal at the point of use is a better answer than an option the type system pretends does not exist.
</Callout>
