...
- **Nested `with` uses batched loaders by default, native joins on request.** PowDB has no `json_agg`/link-navigation, so by default each relation level — including **many-to-many through a junction table** — is a separate batched lookup (keys chunked at 1,000). As of v0.35, passing `relationLoadStrategy: 'join'` (per query or in `turbinePowDB` options) compiles eligible top-level relations to **native server-side PowQL joins** instead — see the joins section below. Output is identical either way.
...
By default PowDB relation loading runs the batched loaders described above. As of v0.35 you can opt eligible relations into **native server-side joins** instead — one hash-accelerated join statement per relation, no key lists, no 1,000-key chunking:
