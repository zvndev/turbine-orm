...
<Callout type="warning" title="An already-open embedded Database handle is not an accepted target">
  Only the `{ embedded: dir }` **descriptor** opens the in-process addon.
  Passing an already-constructed `@zvndev/powdb-embedded` `Database` falls
  through to the **networked** branch, which tries to load
  `@zvndev/powdb-client` and fails with *"requires the optional peer
  @zvndev/powdb-client"*, an error about the wrong transport entirely. Hand
  Turbine the directory and let it open the handle.
</Callout>
