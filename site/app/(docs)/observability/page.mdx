...
To see the real parameter values instead of `[REDACTED]`, opt in with `logQueryParams: true` on the client:
...
Only `event.params` changes. The SQL, timing, model, action, and row count are identical either way, so a listener written against redacted events keeps working.

<Callout type="note" title="Relationship to `errorMessages`">
Before this option existed the switch was [`errorMessages: 'verbose'`](/errors#safe-error-messages), which is not a name anyone searches for when they want to see query params. It still works, and it still reveals params, so nothing that relies on it breaks. `logQueryParams` is an alias resolved from a single value, not a second switch: when you set it, it decides; when you leave it unset, `errorMessages` decides exactly as before.

The combination that was previously unreachable is `logQueryParams: true` with the default `errorMessages: 'safe'`: query events carry real values while `NotFoundError` messages stay redacted.
</Callout>
