...
`$on` differs from [middleware](/queries#middleware): middleware wraps the call and can mutate args and results; `$on` is a passive tap that fires after execution with the final SQL, timing, and outcome.
