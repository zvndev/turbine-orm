...
Per-transaction timeout, enforced client-side: if the callback exceeds `timeoutMs`, Turbine destroys the connection (aborting any in-flight statement server-side) and rolls back:
