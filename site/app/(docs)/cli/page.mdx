...
Studio ships three tabs:

- **Query** — the default. Compose a `findMany` visually: pick fields (`select` / `omit`), filter (`where` with AND/OR/NOT and the full operator set), order and `limit`, and drill into relations (`with`) — recursively, to any depth, picking fields and filters at every level. A live TypeScript preview shows the exact `db.table.findMany(...)` call to copy into your codebase. There is no raw-SQL surface: every query is expressed in Turbine ORM, so what you build is what you ship.
...
Everything still executes inside `BEGIN READ ONLY` with a 30s transaction-local statement timeout (parameterized `set_config`), so writes are impossible at the transaction level.

**Saved queries** persist to `.turbine/studio-queries.json` and surface in the sidebar and the **Cmd+K command palette**, which also jumps to any table or tab in a single keystroke.
...
- No SQL input surface at all — builder requests are validated identifier-by-identifier against the introspected schema, and every value is bound as a `$N` parameter.
- Security headers on every response (CSP, `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`), per-session rate limiting, cross-origin refusal.
