...
Six tools: `schema_overview`, `table_detail`, `migrate_status`, `doctor_report`, `explain_query` (SELECT-only), and `sample_rows` (≤ 50 rows). Every database access runs inside `BEGIN READ ONLY`. `--include` / `--exclude` scope which tables are exposed.
