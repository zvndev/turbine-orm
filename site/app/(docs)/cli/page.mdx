...
`doctor --json` emits a stable, versioned report (`schemaVersion: 1`) covering every finding (tier, metrics, and the create/drop SQL), the invalid indexes, the thresholds, and any degradation notices. The versioned envelope is a contract, so a consumer that parses it will not break when new fields are added.
