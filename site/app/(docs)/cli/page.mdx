...
- `prisma-migration-report.md`: per-model resolution, junction tables detected for implicit many-to-many, enums, unresolved items, and parser notes
...
| `--out`, `-o <dir>` | Output directory (default `./generated/turbine`), which must be inside the project root. |
