...
Since 0.60 the emitted map also records a fingerprint of the `schema.prisma` it was generated from, and `createPrismaCompatClient` warns once at startup, in development only, when the file on disk no longer matches. See [Keeping the map current](/migrate-from-prisma#keeping-the-map-current).
...
Ten tools: `schema_overview`, `table_detail`, `relation_graph`, `find_join_path` (returns the `with` clause to write), `migrate_status`, `doctor_report`, `explain_query` (schema-validated findMany-style args only, no free-form SQL), `table_stats`, `sample_rows` (≤ 50 rows), and `explain_error` (needs no database, so it answers when the pool is unreachable). Every database access runs inside `BEGIN READ ONLY`. `--include` / `--exclude` scope which tables are exposed.
...
<Callout type="warning" title="There is no turbine.config.json">
  Those four filenames are the entire list. A `turbine.config.json` is not
  looked for and not read, so a project that has one gets the built-in defaults
  with no warning at all: `out` silently becomes `./generated/turbine` and any
  `include` / `exclude` you wrote is ignored. Rename it to
  `turbine.config.ts` and `export default`.
</Callout>
