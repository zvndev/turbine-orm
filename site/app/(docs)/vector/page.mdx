...
This compiles to `ORDER BY "embedding" <=> $1::vector ASC`, with the vector bound as a parameter.
...
Distance filters accept `lt`, `lte`, `gt`, and `gte`. A distance filter with no comparator throws `ValidationError`.
