...
| `TURBINE_E015` | `OptimisticLockError` | Version mismatch on an `optimisticLock` update, the row was modified by another transaction |
| `TURBINE_E016` | `ExclusionConstraintError` | pg 23P01, translated via `wrapPgError()` |
| `TURBINE_E017` | `UnsupportedFeatureError` | A Postgres-only feature (pgvector, LISTEN/NOTIFY, RLS `sessionContext`) invoked on a non-Postgres engine, exposes `feature` and `dialect` |
| `TURBINE_E018` | `ReadOnlyError` | A write refused because the database is read-only, exposes `reason: 'snapshot' \| 'rbac'` (PowDB snapshot serving / read-only role) |
