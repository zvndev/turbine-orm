...
| `TURBINE_E003` | `ValidationError` | Unknown column, invalid operator, empty-where guard on update/delete, PII aggregate without `includePii`, a `createMany` whose first data row names no column while a later row does |
...
On PostgreSQL these features never throw, `TURBINE_E017` only appears on the non-Postgres engines. See [Database Engines](/engines) for the full capability matrix.
