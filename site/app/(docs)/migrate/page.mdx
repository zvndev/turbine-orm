...
- **`migrate up`** scans every pending migration for destructive statements. If it finds any, it stops, prints exactly which statements in which files are destructive, and asks for a typed confirmation (`yes`) before applying anything. Decline and nothing runs; no data is touched. You can skip the prompt with `--allow-destructive` when you know what you are doing.
