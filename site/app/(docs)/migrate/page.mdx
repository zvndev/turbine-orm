...
<Callout type="warning" title="Do not write your own BEGIN/COMMIT in a migration">
  ...
  `COMMIT`, `ROLLBACK` or `ABORT` is refused before anything runs. Earlier
  versions accepted it, and that was the bug: the runner wraps each file in one
  transaction, so an embedded `COMMIT` ended the runner's wrapper mid-file.
  Everything before it became durable, everything after ran unprotected, and the
  migration was recorded nowhere, so rerunning failed forever on "already
  exists". If a migration genuinely cannot run inside a transaction (`CREATE
  INDEX CONCURRENTLY`), put `-- turbine:no-transaction` in the file header and
  manage it yourself. `ROLLBACK TO SAVEPOINT` is still allowed: it leaves the
  wrapping transaction open.
</Callout>
