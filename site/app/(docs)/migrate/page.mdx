...
- **Configured schema.** With `schema` set to anything but `public` in `turbine.config.ts`, `migrate up` / `down` / `deploy` / `status` connect with `search_path` pinned to that schema as a connection **startup parameter** (`options=-c search_path="<schema>"`, never a session `SET`), so a migration's unqualified DDL and the `_turbine_migrations` table itself land in the schema `push`, `generate` and `doctor` already read. The value replaces the role's default path rather than extending it, so an object from another schema (an extension type installed in `public`, say) must be qualified. The name must be a plain identifier (letters, digits, `_`, `$`, not starting with a digit); anything else is refused with `TURBINE_E003` before a connection is opened.
