...
| E017 | `UnsupportedFeatureError` | A Postgres-only feature (pgvector, `LISTEN`/`NOTIFY`, RLS `sessionContext`) invoked on an engine that does not support it |
...
npx turbine migrate deploy                # Apply pending without prompts (CI)
...
npx turbine doctor                        # Report relations missing an FK index (--fix emits a migration)
npx turbine studio                        # Local read-only web UI (default port 4983)
npx turbine mcp                           # Read-only MCP server over JSON-RPC stdio
npx turbine observe                       # Metrics dashboard (requires TURBINE_OBSERVE_URL)
```
...
```bash
npx turbine studio
# → http://127.0.0.1:4983/?token=<auto-generated 24-byte hex>
# override with --port <n> --host <addr>
```

Since v0.19 Studio is **ORM-native**: there is no raw-SQL surface at all. Three tabs:

- **Query** (default): a visual `findMany` builder. Pick a table, drill into relations (`with`) to any depth, select/omit fields, add `where` filters, `orderBy`, and `limit` at every level. The server validates every identifier (table, relation, field, orderBy) against the introspected schema and compiles the args with the same `QueryInterface.buildFindMany` your application uses, binding all values as `$N` params.
- **Data**: browse any table with pagination.
- **Schema**: tables, columns, relations, indexes.

Saved queries are builder-kind only; any legacy raw-SQL entries are dropped on load with a console notice.

**Security posture** (port this if you build a PowDB Studio):

1. **Loopback-only bind.** `127.0.0.1`, not `0.0.0.0`. Warns loudly and refuses a non-loopback host unless you pass `--allow-remote`.
2. **Per-process auth token.** 24 random bytes, constant-time compared on every `/api/*` route.
3. **Builder-only surface.** No SQL is ever accepted from the client. Every query is compiled from validated builder args against the introspected schema, so a value can never reach the SQL string un-parameterized.
4. **Engine-level read-only transaction.** Every query runs inside `BEGIN READ ONLY` with `SELECT set_config('statement_timeout', $1, true)` and `set_config('search_path', $1, true)` pinned to the configured schema. Belt-and-suspenders: even if something slipped through, Postgres refuses writes.
5. **Rate limiting plus cross-origin refusal.** Per-session limit (100 requests / 60s) and exact-origin match, not a wildcard.
6. **Security headers.** `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`, plus CSP.

For PowDB: same rules. The "every query in a read-only transaction, composed only from validated builder args" guarantee is the strongest one: the surface physically cannot mutate the database.

---

## 15. Testing patterns

Turbine does not ship a public mocking or test-helper surface. Its own build-only SQL helpers (`mockTable` / `makeQuery`) are internal test utilities, not part of the package exports, so the realistic pattern for your app is to run the normal generated client against a real test database.
...
// assert against `found`, then tear down
```

Because reads and writes go through the real client, these tests catch query-shape and result-parsing regressions that a mock would miss.

### Gating on `DATABASE_URL`

Skip the DB-dependent suites when no database is configured so `npm run test:unit` stays green without one. The exact idiom depends on your runner: with Vitest or Jest, branch on `describe` vs `describe.skip`:
...
With Node's built-in `node:test`, pass `{ skip }` to each test instead. In this repo, `./scripts/seed-test-db.sh` spins up a throwaway Postgres and seeds the fixture so contributors can run the full suite locally.
...
- **Migration guide:** [`docs/migrate-from-prisma.md`](./migrate-from-prisma.md)
