...
describe('powdb F2: native-join emission', () => {
  ...
  it('pushes the relation orderBy into the hasMany join but NEVER its limit (a per-parent bound)', async () => {
    // This test used to assert `limit $n` ON the join statement. That limit
    // bounded the total child count across every parent, so `limit: 3` over
    // ten parents returned three posts shared among them; it read as correct
    // because the loader path did the same. The relation page is applied per
    // parent at stitch time (pageOf), on both paths; the orderBy stays on the
